...
void Awake()
	{ Time.timeScale=1; 
		GetGoldValues ();
		_FacebookButton.renderer.enabled = false;
		// Initialize FB SDK              
	...
	void OnGUI()
	{
		...
		if (EnableFacebook) {
						if (!FB.IsLoggedIn) {
								_FacebookButton.renderer.enabled = true;
						} else {
								_FacebookButton.renderer.enabled = false;
						}
	...
	void Update () {
		...
		if (Input.GetButtonDown ("Fire1") ) {
			...
			{  if(( hit.transform.gameObject.name ==_FacebookButton.name )  ){  
				...
				else if(( hit.transform.gameObject.name ==_PlayButton.name )  ){audio.PlayOneShot(MenuSound);Util.ButtonPressAnimation(hit.transform.gameObject); Application.LoadLevel(_NextScene);Time.timeScale=1; }
				else if(( hit.transform.gameObject.name ==_PurchaseButton.name )  ){audio.PlayOneShot(MenuSound);Util.ButtonPressAnimation(hit.transform.gameObject);TransitToPowerMenu(); }
				else if(( hit.transform.gameObject.name ==_TransitionPlayButton.name )  ){audio.PlayOneShot(MenuSound); Util.ButtonPressAnimation(hit.transform.gameObject);Application.LoadLevel(_NextScene);Time.timeScale=1; }
				else if(( hit.transform.gameObject.name ==_TransitionReturnButton.name )  ){audio.PlayOneShot(MenuSound);Util.ButtonPressAnimation(hit.transform.gameObject);TransitMainMenu(); }
				else if(( hit.transform.gameObject.name ==_AddCoinsButton.name )  ){audio.PlayOneShot(MenuSound); Util.ButtonPressAnimation(hit.transform.gameObject);TransitToStoreMenu(); }
				else if(( hit.transform.gameObject.name ==_StoreReturnButton.name )  ){audio.PlayOneShot(MenuSound); Util.ButtonPressAnimation(hit.transform.gameObject);TransitToPowerMenu(); }
				else if(( hit.transform.gameObject.name ==_50CoinsButton.name )  ){audio.PlayOneShot(MenuSound);Util.ButtonPressAnimation(hit.transform.gameObject);IncreaseGoldValue(50); }
				else if(( hit.transform.gameObject.name ==_100CoinsButton.name )  ){audio.PlayOneShot(MenuSound); Util.ButtonPressAnimation(hit.transform.gameObject);IncreaseGoldValue(100); }
				else if(( hit.transform.gameObject.name ==_200CoinsButton.name )  ){audio.PlayOneShot(MenuSound); Util.ButtonPressAnimation(hit.transform.gameObject);IncreaseGoldValue(200); }
				else if(( hit.transform.gameObject.name ==_300CoinsButton.name )  ){audio.PlayOneShot(MenuSound); Util.ButtonPressAnimation(hit.transform.gameObject);IncreaseGoldValue(300); }
				else if(( hit.transform.gameObject.name ==_MoreTimeButton.name ) && DecreaseGoldValue(10) ){audio.PlayOneShot(MenuSound); Util.ButtonPressAnimation(hit.transform.gameObject);if(! _IsMoreTime  ){ _IsMoreTime = true ;PlayerPrefs.SetInt("MoreTime", 1);  DisableButton(hit.transform.gameObject);DisableButton(_MoreTimeButtonText);}}
				else if(( hit.transform.gameObject.name ==_LuckButton.name)&& DecreaseGoldValue(10)){audio.PlayOneShot(MenuSound); Util.ButtonPressAnimation(hit.transform.gameObject);if(! _IsLuck  ){_IsLuck = true ;PlayerPrefs.SetInt("Luck", 1);DisableButton(hit.transform.gameObject);DisableButton(_LuckButtonText);}}
				else if(( hit.transform.gameObject.name ==_Luckx2Button.name )&& DecreaseGoldValue(10)  ){audio.PlayOneShot(MenuSound); Util.ButtonPressAnimation(hit.transform.gameObject);if(! _IsLuckx2 ){_IsLuckx2 = true ;PlayerPrefs.SetInt("Luckx2", 1);DisableButton(hit.transform.gameObject);DisableButton(_Luckx2ButtonText);} }

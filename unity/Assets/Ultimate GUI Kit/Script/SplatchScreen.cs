...
public class SplatchScreen : MonoBehaviour
{
		...
		{    	
				...
				_Logo.renderer.material.color = new Color (oldColor.r, oldColor.b, oldColor.g, 0f);

				parms = new TweenParms ().Prop ("color", new Color (oldColor.r, oldColor.b, oldColor.g, 1f)).Ease (EaseType.EaseInQuart);

				mySequence.Append (HOTween.To (_Logo.renderer.material, _FadeInTime, parms));
				mySequence.Append (HOTween.To (_Logo.renderer.material, _WaitingTime, parms));

				parms = new TweenParms ().Prop ("color", new Color (oldColor.r, oldColor.b, oldColor.g, 0f));

				mySequence.Append (HOTween.To (_Logo.renderer.material, _FadeOutTime, parms));
